...
public class EditarPerfilActivity extends AppCompatActivity {
    private EditText editTextNombre, editTextApellido,editTextContrasenia;
    ...
    private void obtenerContraseniaRegistrada() {
        userRef.child("contrasena").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
        ...
        builder.setPositiveButton("OK", (dialog, which) -> {
            ...
            auth.getCurrentUser().updatePassword(nuevaContrasenia).addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    userRef.child("contrasena").setValue(nuevaContrasenia);
